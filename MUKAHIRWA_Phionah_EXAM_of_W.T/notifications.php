...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <label for="NotificationContent">NotificationContent:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO notifications(NotificationID, UserID, NotificationType, NotificationContent, IsRead,Timestamp) VALUES (?, ?, ?, ?, ? ,?)");
    $stmt->bind_param("isssss", $NotificationID, $UserID, $NotificationType, $NotificationContent, $IsRead,$Timestamp);
    // Set parameters and execute
    $FeedbackID = $_POST['driver_id'];
    $SessionID = $_POST['user_id'];
    $ClientID = $_POST['license_number'];
    $TherapistID = $_POST['car_model'];
    $Rating = $_POST['capacity'];
    $FeedbackContent = $_POST['capacit'];
    $FeedbackDate = $_POST['capaci'];
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
            ...
            <td>" . $row['IsRead'] . "</td>
               <td>" . $row['Timestamp'] . "</td>
            <td><a style='padding:4px' href='delete_notifications.php?NotificationID=$NotificationID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_notifications.php?NotificationID=$NotificationID'>Update</a></td> 
        </tr>";
    }
